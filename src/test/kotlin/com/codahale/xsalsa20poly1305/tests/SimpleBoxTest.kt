...
internal class SimpleBoxTest : WithQuickTheories {
    ...
    @Test
    fun pkRoundTrip() {
        qt().forAll(Generators.privateKeys(), Generators.privateKeys(), Generators.byteArrays(1, 4096))
    ...
    @Test
    fun shortMessage() {
        qt().forAll(Generators.byteArrays(32, 32), Generators.byteArrays(1, 24))
            .check { key, message -> !SimpleBox(key).open(message) != null }
    }
}