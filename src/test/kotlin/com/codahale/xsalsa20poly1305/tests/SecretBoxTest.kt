...
internal class SecretBoxTest : WithQuickTheories {
    @Test
    fun shortKey() {
        qt().forAll(Generators.byteArrays(1, 31)).checkAssert { key ->
    ...
    fun roundTrip() {
        qt()
            ...
            .check { key, nonce, message ->
                ...
                box.open(nonce, box.seal(nonce, message)).let { a -> Arrays.equals(message, a) }
            }
    }
    ...
    fun pkRoundTrip() {
        qt()
            ...
            .check { privateKeyA, privateKeyB, nonce, message ->
                ...
                assertNotNull(message)
                boxB.open(nonce, boxA.seal(nonce, message)).let { a -> Arrays.equals(message, a) }
            }
    }
    ...
    fun badCiphertext() {
        qt()
            ...
            .check { key, nonce, message, v ->
                ...
                ciphertext[v % ciphertext.size] =
                    (ciphertext[v % ciphertext.size].toInt() xor mask.toInt()).toByte()
    ...
    fun randomNonce() {
        ...
        val nonces =
            IntStream.range(0, 1000).mapToObj { _: Int -> box.nonce() }.collect(Collectors.toList())
        qt()
    ...
    fun fromLibSodiumToUs() {
        qt()
            ...
            .check { key, nonce, message ->
                val c: ByteArray =
                    org.abstractj.kalium.crypto.SecretBox(key).encrypt(nonce, message)
