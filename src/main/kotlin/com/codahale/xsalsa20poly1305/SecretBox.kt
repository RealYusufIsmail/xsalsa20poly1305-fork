...
class SecretBox(secretKey: ByteArray) {
     ...
     * @return an [Optional] of the original plaintext, or if either the key, nonce, or ciphertext
     * was modified, an empty [Optional]
     * @see .nonce
     * @see .nonce
     */
    fun open(nonce: ByteArray?, ciphertext: ByteArray): ByteArray? {
        ...
        // hash ciphertext
        poly1305.init(KeyParameter(sk))
        val len = (ciphertext.size - poly1305.macSize).coerceAtLeast(0)
     ...
     * attacker to potentially decrypt and even forge messages), [.nonce] is recommended instead.
     *
     ...
     *
     * In the event of a compromised [SecureRandom] provider, the attacker would need a complete
